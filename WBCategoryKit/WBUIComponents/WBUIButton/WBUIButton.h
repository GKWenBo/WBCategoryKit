...
typedef NS_ENUM(NSUInteger, WBUIButtonImagePosition) {
    WBUIButtonImagePositionTop,             // imageView在titleLabel上面
    WBUIButtonImagePositionLeft,            // imageView在titleLabel左边
    WBUIButtonImagePositionBottom,          // imageView在titleLabel下面
    WBUIButtonImagePositionRight,           // imageView在titleLabel右边
};
 ...
 * @warning WBUIButton 重新定义了 UIButton.titleEdgeInests、imageEdgeInsets、contentEdgeInsets 这三者的布局逻辑，sizeThatFits: 里会把 titleEdgeInests 和 imageEdgeInsets 也考虑在内（UIButton 不会），以使这三个接口的使用更符合直觉。
 */
@interface WBUIButton : UIButton
...
/// 是否自动调整highlighted时的按钮样式，默认为YES。<br/> 当值为YES时，按钮highlighted时会改变自身的alpha属性为<b>ButtonHighlightedAlpha</b>
...
/// 设置按钮点击时的背景色，默认为nil。
/// @warning 不支持带透明度的背景颜色。当设置highlightedBackgroundColor时，会强制把adjustsButtonWhenHighlighted设为NO，避免两者效果冲突。 @see adjustsButtonWhenHighlighted
...
@property(nonatomic, assign) WBUIButtonImagePosition imagePosition;
