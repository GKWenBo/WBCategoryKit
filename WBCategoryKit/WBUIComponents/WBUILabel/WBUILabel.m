...
- (CGSize)sizeThatFits:(CGSize)size {
    size = [super sizeThatFits:CGSizeMake(size.width - WBUIEdgeInsetsGetHorizontalValue(self.contentEdgeInsets), size.height - WBUIEdgeInsetsGetVerticalValue(self.contentEdgeInsets))];
    size.width += WBUIEdgeInsetsGetHorizontalValue(self.contentEdgeInsets);
    size.height += WBUIEdgeInsetsGetVerticalValue(self.contentEdgeInsets);
    return size;
}
...
- (void)drawTextInRect:(CGRect)rect {
    ...
    if (self.numberOfLines == 1 && (self.lineBreakMode == NSLineBreakByWordWrapping || self.lineBreakMode == NSLineBreakByCharWrapping)) {
        rect = WBCGRectSetHeight(rect, CGRectGetHeight(rect) + self.contentEdgeInsets.top * 2);
    }
