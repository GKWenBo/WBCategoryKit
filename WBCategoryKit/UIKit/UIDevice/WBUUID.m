...
NSString *const WBUUIDsOfUserDevicesDidChangeNotification = @"WBUUIDsOfUserDevicesDidChangeNotification";

static NSString *const _uuidForInstallationKey = @"wb_uuidForInstallation";
static NSString *const _uuidForDeviceKey = @"wb_uuidForDevice";
static NSString *const _uuidsOfUserDevicesKey = @"wb_uuidsOfUserDevices";
static NSString *const _uuidsOfUserDevicesToggleKey = @"wb_uuidsOfUserDevicesToggle";

@implementation WBUUID

+ (instancetype)sharedInstance {
    static WBUUID *instance = nil;
...
{
    NSString *value = [self _getValueForKey:key
...
- (NSString *)_getValueForKey:(NSString *)key userDefaults:(BOOL)userDefaults keychain:(BOOL)keychain service:(NSString *)service accessGroup:(NSString *)accessGroup
{
    ...
    if(!value && keychain ){
        value = [UICKeyChainStore stringForKey:key
                                       service:service
                                   accessGroup:accessGroup];
    }
...
- (void)uuidForDevice_updateWithValue:(NSString *)value
{
    ...
    [self _setValue:_uuidForDevice
             forKey:_uuidForDeviceKey
       userDefaults:YES
           keychain:YES
            service:nil
        accessGroup:nil
...
- (NSString *)uuidForDeviceMigratingValueForKey:(NSString *)key
                                ...
                                commitMigration:(BOOL)commitMigration
{
    NSString *uuidToMigrate = [self _getValueForKey:key
                                       userDefaults:YES
                                           keychain:YES
                                            service:service
                                        accessGroup:accessGroup];
    
    return [self uuidForDeviceMigratingValue:uuidToMigrate
                             commitMigration:commitMigration];
}
...
- (void)uuidsOfUserDevices_iCloudSync
{
    if( _uuidsOfUserDevices_iCloudAvailable )
    {
        ...
        [iCloud setBool:![iCloud boolForKey:_uuidsOfUserDevicesToggleKey]
...
- (void)uuidsOfUserDevices_iCloudChange:(NSNotification *)notification
{
    if( _uuidsOfUserDevices_iCloudAvailable )
    {
        ...
        if([uuidsSet count] > uuidsCount)
        {
            ...
            [[NSNotificationCenter defaultCenter] postNotificationName:WBUUIDsOfUserDevicesDidChangeNotification object:self userInfo:userInfo];
        }
    }
}
...
- (void)uuidsOfUserDevices_updateWithValue:(NSArray *)value
{
    ...
    [self _setValue:_uuidsOfUserDevices
             forKey:_uuidsOfUserDevicesKey
       userDefaults:YES
           keychain:YES
            service:nil
        accessGroup:nil
...
+ (NSString *)uuidForDeviceMigratingValue:(NSString *)value
                          commitMigration:(BOOL)commitMigration
{
    return [[self sharedInstance] uuidForDeviceMigratingValue:value
                                              commitMigration:commitMigration];
}
